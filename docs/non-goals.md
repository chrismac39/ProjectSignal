# Non-Goals

> Status: North-star guardrails. Current exclusions are defined in [Prototype 0 Specification](prototype-spec.md).

## Enduring Non-Goals

ProjectSignal is not intended to become:

* A conventional perfect-information RTS.
* A unit-micro or actions-per-minute test.
* A symmetrical competitive faction game.
* A territory-painting or map-completion loop.
* A Factorio-style throughput optimization game.
* A technology race where larger numbers replace interpretation.
* A notification system that identifies important events and recommends responses.
* An esports-balance target where faction differences must converge on equivalent tools.
* A combat-first game in which reconnaissance only supplies targeting bonuses.

## Not Rejected, But Deferred

The following may eventually support the information game, but are outside Prototype 0:

* Combat resolution.
* Production and logistics economies.
* Interactive command input.
* AI opponents.
* Multiplayer.
* Strategic victory conditions.
* Detailed geography and movement.
* Institutional adaptation.
* A presentation or visualization client.

Deferral is not a promise that these systems will be built. Each must first pass the design test in the [North Star](north-star.md).